...
;; Trying to change a non-root non-top-level contact's scale changes
;; is an error.
(defmethod (setf contact-scale) (new-value (self contact))
  (declare (ignore new-value))
  (error "~s inherits scale from its ~:[top-level~;root~] ancestor."
	 self (top-level-p self)))

;; Once a top-level shell's scale has been changed, propagate the
;; effect of the change to all its descendents...
...
(defmethod initialize-instance :after ((contact core) &rest initargs)
  ...
  (with-slots (foreground) contact
	...
	(setf foreground (or (inherited-foreground contact)
			     (convert contact *default-contact-foreground* 'clue::pixel)))
	nil
...
(defmethod (setf contact-border) (new-border (contact core))
  (with-slots (border) contact
    (let ((converted-border
	   (convert contact new-border '(or pixel pixmap (member :copy)))))
      (assert converted-border nil
	      "~a cannot be converted to PIXEL, PIXMAP, or :COPY."
	      new-border)
...
(defmethod (setf contact-foreground) (new-foreground (contact core))
  (with-slots (foreground) contact
    (let ((converted-foreground (convert contact new-foreground 'pixel)))
      (assert converted-foreground
	      nil
	      "~a cannot be converted to a PIXEL."
	      new-foreground)
...
(defparameter *open-look-scale-fontnames*
  ...
  "These are the fontnames used to implement Open Look, if available.
   Modify these if you want to override CLIO's use of Open Look
   fonts.")
...
(defmethod convert ((contact contact) (value (eql :default))
		    (type (eql 'default-horizontal-space)))
  (point-pixels (contact-screen contact) *default-display-horizontal-space*))

(defmethod convert ((contact contact) (value (eql :default))
		    (type (eql 'default-vertical-space)))
...
(defmethod (setf display-horizontal-space) (new-value (contact spacing-mixin))
  (with-slots (horizontal-space) contact
    ...
    (setf horizontal-space
	  (if (eq :default new-value)
